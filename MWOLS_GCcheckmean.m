function [MWRMSEallcheckmean,MWRMSEallallmean,MWRMSEallallmean2,RMSEall,OLS_percent,RMSEGC,RMSEallfirst]= MWOLS_GCcheckmean(GC,sample,lipidlibrary,compnum,range,step);
WNstr=80;
%WNstp=273;
count=1;
...
OLS(1:n,:)=C(j+1:n+j,:);%%create OLS conc. matrix
    ...
    meanOLS(1,:)=mean(OLS_percent(1:n,:));
    
    for x=1:n
        for y=1:j
            RMSEGC(x,y)=GCdata(1,y)-meanOLS(1,y);
        end
    end
...
RMSEall(n+1,count)=WNstr_step;
 ...
 for x=1:n
        ...
        [testa1,testb1]=find(RMSEall(x,1:fin_count)<testmean(1,1)*2/4);%selection criteria
        ...
        elseif tb1m>1
            for i=1:tb1m
                MWRMSEallallmean(x,i)=RMSEall(x,testb1(1,i));%MWRMSEallallmean(x+x+n+2-1,i)=RMSEall(x,testb1(1,i));
                MWRMSEallallmean(x+1,i)=RMSEall(n+1,testb1(1,i));%MWRMSEallallmean(x+x+n+2,i)=RMSEall(n+1,testb1(1,i));  
            end 
        end
  end

   MWRMSEallcheckmean=RMSEall;
   
   [m1,n1]=size(MWRMSEallallmean);
     for x=1:m1-1 
        for y=n1:-1:1
            if MWRMSEallallmean(x,y)==0
            else
         
            MWRMSEallallmean2(x,1)=mean(MWRMSEallallmean(x,1:y));
            break
            end
    ...
    MWRMSEallallmean2(m1+2,1)=mean(MWRMSEallallmean2(1:m1-1,1));
end
